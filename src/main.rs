...
use candle_core::{Device, Tensor, DType, Result, Shape};
...
fn draw(data: Vec<f32>) {
  match model::load_weights("assets/weights.safetensors") {
    Ok(weights) => {
      println!("✓ Weights loaded successfully!");

      let device = &Device::Cpu;
      let input_4d = Tensor::from_vec(data, (1, 1, 500, 500), &device).unwrap();
      let resized_4d = input_4d.interpolate2d(28, 28).unwrap();
      let input = resized_4d.flatten_all().unwrap().reshape((1,784)).unwrap();
      match model::model_forward(&weights, &input) {
        Ok(output) => {
          ...
          set_prediction.set(digit.to_string());
        }
